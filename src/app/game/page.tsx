...
function isGameOver(word: string, guessedLetters: string[]): boolean {
  return isWordGuessed(word, guessedLetters) || countWrongGuesses(word, guessedLetters) >= 6;
}
...
export default function GamePage() {
  ...
  useEffect(() => {
    async function loadWords() {
      try {
        const response = await fetch(`/api/words?difficulty=${currentDifficulty}`);
        ...
        setWords(fallbackWords[currentDifficulty as keyof typeof fallbackWords]);
        setCurrentWord(getRandomWord(fallbackWords[currentDifficulty as keyof typeof fallbackWords]));
        setIsLoading(false);
      }
    }
  ...
  const handleKeyPress = (key: string) => {
    ...
    if (!guessedLetters.includes(key)) {
      ...
      if (isWordGuessed(currentWord, newGuessedLetters) || 
      ...
      <header className="flex justify-between items-center mb-8">
        ...
        <div className="text-center">
          ...
          <div className="text-sm font-medium mt-1">
            {currentDifficulty === 'easy' && <span className="text-green-500">Easy Mode (4 letters)</span>}
            {currentDifficulty === 'medium' && <span className="text-blue-500">Medium Mode (5 letters)</span>}
            {currentDifficulty === 'hard' && <span className="text-red-500">Hard Mode (8 letters)</span>}
          </div>
        </div>
